...
static void sha256_crypt(__private const unsigned int *pass, int pass_len, __private unsigned int* hash)    \
{                                   \
    int plen=pass_len/4;            \
...
typedef uint32_t bignum25519[16];
typedef uint32_t bignum25519align16[16];
...
typedef bignum256modm_element_t bignum256modm[16];
...
curve25519_mul(__private bignum25519 out, __private const bignum25519 a, __private const bignum25519 b) {
	...
	uint32_t s[16];//0,s1,s2,s3,s4,s5,s6,s7,s8,s9;
	uint64_t m[16];//0,m1,m2,m3,m4,m5,m6,m7,m8,m9;
    uint64_t c;
	uint32_t p;
	...
	m[1] = mul32x32_64(r0, s[1]) + mul32x32_64(r1, s[0]);
	m[3] = mul32x32_64(r0, s[3]) + mul32x32_64(r1, s[2]) + mul32x32_64(r2, s[1]) + mul32x32_64(r3, s[0]);
	m[5] = mul32x32_64(r0, s[5]) + mul32x32_64(r1, s[4]) + mul32x32_64(r2, s[3]) + mul32x32_64(r3, s[2]) + mul32x32_64(r4, s[1]) + mul32x32_64(r5, s[0]);
	m[7] = mul32x32_64(r0, s[7]) + mul32x32_64(r1, s[6]) + mul32x32_64(r2, s[5]) + mul32x32_64(r3, s[4]) + mul32x32_64(r4, s[3]) + mul32x32_64(r5, s[2]) + mul32x32_64(r6, s[1]) + mul32x32_64(r7, s[0]);
	m[9] = mul32x32_64(r0, s[9]) + mul32x32_64(r1, s[8]) + mul32x32_64(r2, s[7]) + mul32x32_64(r3, s[6]) + mul32x32_64(r4, s[5]) + mul32x32_64(r5, s[4]) + mul32x32_64(r6, s[3]) + mul32x32_64(r7, s[2]) + mul32x32_64(r8, s[1]) + mul32x32_64(r9, s[0]);
	...
	m[0] = mul32x32_64(r0, s[0]);
	m[2] = mul32x32_64(r0, s[2]) + mul32x32_64(r1, s[1]) + mul32x32_64(r2, s[0]);
	m[4] = mul32x32_64(r0, s[4]) + mul32x32_64(r1, s[3]) + mul32x32_64(r2, s[2]) + mul32x32_64(r3, s[1]) + mul32x32_64(r4, s[0]);
	m[6] = mul32x32_64(r0, s[6]) + mul32x32_64(r1, s[5]) + mul32x32_64(r2, s[4]) + mul32x32_64(r3, s[3]) + mul32x32_64(r4, s[2]) + mul32x32_64(r5, s[1]) + mul32x32_64(r6, s[0]);
	m[8] = mul32x32_64(r0, s[8]) + mul32x32_64(r1, s[7]) + mul32x32_64(r2, s[6]) + mul32x32_64(r3, s[5]) + mul32x32_64(r4, s[4]) + mul32x32_64(r5, s[3]) + mul32x32_64(r6, s[2]) + mul32x32_64(r7, s[1]) + mul32x32_64(r8, s[0]);
	...
	m[1] += (mul32x32_64(r9, s[2]) + mul32x32_64(r8, s[3]) + mul32x32_64(r7, s[4]) + mul32x32_64(r6, s[5]) + mul32x32_64(r5, s[6]) + mul32x32_64(r4, s[7]) + mul32x32_64(r3, s[8]) + mul32x32_64(r2, s[9]));
	m[3] += (mul32x32_64(r9, s[4]) + mul32x32_64(r8, s[5]) + mul32x32_64(r7, s[6]) + mul32x32_64(r6, s[7]) + mul32x32_64(r5, s[8]) + mul32x32_64(r4, s[9]));
	m[5] += (mul32x32_64(r9, s[6]) + mul32x32_64(r8, s[7]) + mul32x32_64(r7, s[8]) + mul32x32_64(r6, s[9]));
	m[7] += (mul32x32_64(r9, s[8]) + mul32x32_64(r8, s[9]));
	...
	m[0] += (mul32x32_64(r9, s[1]) + mul32x32_64(r8, s[2]) + mul32x32_64(r7, s[3]) + mul32x32_64(r6, s[4]) + mul32x32_64(r5, s[5]) + mul32x32_64(r4, s[6]) + mul32x32_64(r3, s[7]) + mul32x32_64(r2, s[8]) + mul32x32_64(r1, s[9]));
	m[2] += (mul32x32_64(r9, s[3]) + mul32x32_64(r8, s[4]) + mul32x32_64(r7, s[5]) + mul32x32_64(r6, s[6]) + mul32x32_64(r5, s[7]) + mul32x32_64(r4, s[8]) + mul32x32_64(r3, s[9]));
	m[4] += (mul32x32_64(r9, s[5]) + mul32x32_64(r8, s[6]) + mul32x32_64(r7, s[7]) + mul32x32_64(r6, s[8]) + mul32x32_64(r5, s[9]));
	m[6] += (mul32x32_64(r9, s[7]) + mul32x32_64(r8, s[8]) + mul32x32_64(r7, s[9]));
	m[8] += (mul32x32_64(r9, s[9]));

	                             r0 = (uint32_t)m[0] & reduce_mask_26; c = (m[0] >> 26);
	m[1] += c;                     r1 = (uint32_t)m[1] & reduce_mask_25; c = (m[1] >> 25);
	m[2] += c;                     r2 = (uint32_t)m[2] & reduce_mask_26; c = (m[2] >> 26);
	m[3] += c;                     r3 = (uint32_t)m[3] & reduce_mask_25; c = (m[3] >> 25);
	m[4] += c;                     r4 = (uint32_t)m[4] & reduce_mask_26; c = (m[4] >> 26);
	m[5] += c;                     r5 = (uint32_t)m[5] & reduce_mask_25; c = (m[5] >> 25);
	m[6] += c;                     r6 = (uint32_t)m[6] & reduce_mask_26; c = (m[6] >> 26);
	m[7] += c;                     r7 = (uint32_t)m[7] & reduce_mask_25; c = (m[7] >> 25);
	m[8] += c;                     r8 = (uint32_t)m[8] & reduce_mask_26; c = (m[8] >> 26);
	m[9] += c;                     r9 = (uint32_t)m[9] & reduce_mask_25; p = (uint32_t)(m[9] >> 25);
	m[0] = r0 + mul32x32_64(p,19); r0 = (uint32_t)m[0] & reduce_mask_26; p = (uint32_t)(m[0] >> 26);
	r1 += p;
...
__constant uchar ALIGN(16) gexp[] = {
...
static inline uchar gmult(uchar a, uchar b) {
    if (a == 0 || b == 0) {
        return 0;
    }
...
#define chBaseLoop(a) dividend = (uint8_t)(remaind / divisor); \
    remaind %= divisor; \
    codeword[a] = dividend; \
    divisor /= 32; 
#define calculateP() fb = dividend ^ p[3]; \
    p[3] = p[2] ^ gmult(30, fb); \
    p[2] = p[1] ^ gmult(6, fb); \
    p[1] = p[0] ^ gmult(9, fb); \
    p[0] = gmult(17, fb);

static void idToByteAccount(uint64_t accountId, uint8_t * out) {
...
__kernel void process (
    ...
    for (size_t i = 0; i < passphraseLength; i++) {
        if (i % 4 == 0) {
            uintPass[i / 4] = (uint)  passphrase[thread].string[passphrase[thread].offset + i];
        } else {
            uintPass[i / 4] |= (uint) passphrase[thread].string[passphrase[thread].offset + i] << (8 * (i % 4));
        }
    }
    sha256_crypt(uintPass, passphraseLength, passwordHash);

    U32TO8_LE(privateKey, passwordHash[0]);
    U32TO8_LE(privateKey + 4, passwordHash[1]);
    U32TO8_LE(privateKey + 8, passwordHash[2]);
    U32TO8_LE(privateKey + 12, passwordHash[3]);
    U32TO8_LE(privateKey + 16, passwordHash[4]);
    U32TO8_LE(privateKey + 20, passwordHash[5]);
    U32TO8_LE(privateKey + 24, passwordHash[6]);
    U32TO8_LE(privateKey + 28, passwordHash[7]);
    ...
    uintPass[0] = U8TO32_LE(publicKey +  0);
	uintPass[1] = U8TO32_LE(publicKey +  4);
	uintPass[2] = U8TO32_LE(publicKey +  8);
	uintPass[3] = U8TO32_LE(publicKey + 12);
	uintPass[4] = U8TO32_LE(publicKey + 16);
	uintPass[5] = U8TO32_LE(publicKey + 20);
	uintPass[6] = U8TO32_LE(publicKey + 24);
	uintPass[7] = U8TO32_LE(publicKey + 28);
    ...
    for (size_t i = 0; i < RS_ADDRESS_BYTE_SIZE; i++) {
        ...
        case MASK_MATCH_ANY_NUMBER:
            if (byteRsAccount[i] >= MASK_MATCH_A) return;
            continue;
        ...
        case MASK_MATCH_PREVIOUS:
            if (byteRsAccount[i] != byteRsAccount[i-1]) return;
            continue;
        default:
            if (dataMask[i] != byteRsAccount[i]) return;
        }
    }
    digest2[thread] = 1;
}
