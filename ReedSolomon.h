...
const unsigned char gexp[] = {1, 2, 4, 8, 16, 5, 10, 20, 13, 26, 17, 7, 14, 28, 29, 31, 27, 19, 3, 6, 12, 24, 21, 15, 30, 25, 23, 11, 22, 9, 18, 1};
const unsigned char glog[] = {0, 0, 1, 18, 2, 5, 19, 11, 3, 29, 6, 27, 20, 8, 12, 23, 4, 10, 30, 17, 7, 22, 28, 26, 21, 25, 9, 16, 13, 14, 24, 15};
const unsigned char cwmap[] = {3, 2, 1, 0, 7, 6, 5, 4, 13, 14, 15, 16, 12, 8, 9, 10, 11};
...
void idTOByteAccount(unsigned long accountId, unsigned char * out) {
    unsigned char codeword[BYTE_ADDRESS_SIZE];
    const unsigned char base32Length = 13;
    ...
    for (i = base32Length-1; i<base32Length; i--) {
        unsigned char fb = codeword[i] ^ p[3];
        p[3] = p[2] ^ gmult(30, fb);
        p[2] = p[1] ^ gmult(6, fb);
        p[1] = p[0] ^ gmult(9, fb);
        p[0] = gmult(17, fb);
...
void idTOAccount(unsigned long accountId, char * out) {
    unsigned char byteOut[BYTE_ADDRESS_SIZE];
    ...
    out[ADDRESS_SIZE - 1] = '\0';
}

unsigned char RScharToIndex(char in) {
    switch (in)
    {
    case '2': return 0;
    case '3': return 1;
...
void maskToByteMask(char * charMask, unsigned char * byteMask) {
...
int matchMask(unsigned char * byteMask, unsigned char * account) {
    for (size_t i=0; i<17; i++) {
